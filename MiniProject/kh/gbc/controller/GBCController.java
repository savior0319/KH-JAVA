...
public class GBCController implements Runnable {
	...
	public static GBCController getInstance() {
		return gbcController;
	}
	...
	public boolean checkTargetting() {
		return destFeed == null;
	}

	public void setGBCState() {
		int basicSizeW = growSizeW[gbcData.getLevel() - 1]
				+ (int) (currentWeight * growMultiple[gbcData.getLevel() - 1]);
		int basicSizeH = growSizeH[gbcData.getLevel() - 1]
				+ (int) (currentWeight * growMultiple[gbcData.getLevel() - 1]);
		switch (gbcData.getLevel()) {
		case 1:
			gbcImage = resRoad.getGcgLv1_Img();
			break;
		case 2:
			gbcImage = resRoad.getGcgLv2_Img();
			break;
		case 3:
			gbcImage = resRoad.getGcgLv3_Img();
			break;
		case 4:
			gbcImage = resRoad.getGcgLv4_Img();
			break;
		}
	...
	public void sendCurrentWeight(int addData) {
		if (gbcData.getLevel() == 1)
			lv.updateProgress(0, needWeight[gbcData.getLevel() - 1], addData);
		else
			lv.updateProgress(needWeight[gbcData.getLevel() - 2], needWeight[gbcData.getLevel() - 1], addData);
	}
	...
	public void followObj() {
		...
		timer = new Timer(moveSpeed[gbcData.getLevel() - 1], new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (destFeed != null) {
					if ((Math.abs(imageLabel.getLocation().x - destFeed.getPos().x) <= 10)
							&& (Math.abs(imageLabel.getLocation().y - destFeed.getPos().y) <= 10))// (Math.abs(direc.x -
																									// xPos) <= 1 ) &&
																									// (Math.abs(direc.y
																									// - yPos) <= 1))
					{
